...
% BLANK SPACE.
% http://delivery.acm.org/10.1145/2810000/2809462/p181-ribas.pdf?ip=150.164.3.7&id=2809462&acc=ACTIVE%20SERVICE&key=344E943C9DC262BB%2E7F628E3FAF8B9462%2E4D4702B0C3E38B35%2E4D4702B0C3E38B35&__acm__=1566856133_4390837b2aa1945e1dc3d4ffe5cafa0d

The validity of P-score as a reputation gauge was discussed in the article by Ribas et. al. [!].
P-score's effectiveness as a method of venue ranking was attested by comparing its
performance to citation based indices such as total citations and the H-index using ground truth 
data obtained from the Qualis system maintained
by CAPES, a government agency linked to the Brazilian Ministry of Education. CAPES1
assigns a committee of experts to each area of knowledge and these experts are responsible 
for evaluating all information acquired about thevenues and produce a classification. This classification is
updated annually and follows a set of criteria, such as: the
number of publications in each venue, the number of repositories in which it is indexed, 
the amount of institutions publishing in it, citation information whenever available, among
others. P-score scored notably higher than the other indices in terms of its nDCG compared to the
Qualis ranking. Naturally, to provide that this a proof of P-scores effectiveness, we must accept
that the Qualis ranking is a measure of reputation. This ranking is carefully produced by a
comittee of specialists, but a different comittee could yield a different ranking.
Most researchers can probably agree about the relative reputations of some top conferences
in their field of expertise but the matter gets trickier as we move to less widely known
conferences or those that overlap many fields of knowledge.
In fact, any ground truth could be questioned to some extent since the concept of reputation
cannot be defined objectively.

For this reason, instead of relying on a precise
definition of reputation, P-score takes an agnostic view, exploiting the transference of this
quantity called reputation among entities in order to identify the most reputable ones. The
concept of reputation is determined by the choice of a reference group, which is presumed
to be composed of reputable entities that act as sources of reputation. P-score relies on two
assumptions: 1) a research group conveys reputation to a publication venue proportionally
to its own reputation. 2) a publication venue conveys reputation to a research group propor-
tionally to its own reputation. Given that these are true, it follows from the model definition
that venues will be ranked according to their reputation. In the context of this work, all 
P-score rankings were produced using a reference group of CS departments that are generally 
considered to be prestigious. It follows from P-score's definition that the rankings will
...
% The concept of reputation is hard to define. 
% For this reason, instead of relying on a precise
% definition of reputation, P-score takes an agnostic view, exploiting the transference of this
% quantity called reputation among entities in order to identify the most reputable ones. The
% concept of reputation is determined by the choice of a reference group, which is presumed
...
% to its own reputation. 2) a publication venue conveys reputation to a research group propor-
% tionally to its own reputation. Given that these are true, it follows from the model definition
% that venues will be ranked according to their reputation.

% The concept of reputation is hard to define. For this reason, instead of relying on a precise
% definition of reputation, P-score takes an agnostic view, exploiting the transference of this
% quantity called reputation among entities in order to identify the most reputable ones. The
% concept of reputation is determined by the choice of a reference group, which is presumed
% to be composed of reputable entities that act as sources of reputation. P-score relies on two
...
% The concept of reputation is hard to define and it is also difficult to imagine a definition
% that will be consensual among all evaluators when ranking venue A over venue B. So, instead
% of relying on a precise definition of reputation, P-score takes an agnostic view, exploiting
% the transference of a quantity called reputation among entities in order to identify the most
% reputable ones. The concept of reputation is determined by the choice of reputation sources,
% which depends on the use case. A funding institution may select their preferred group of
% reputable researchers as reputation sources, and P-score will attribute reputations to venues
% according to the publications patterns of these researchers expressed in a reputation flows
% graph.

% P-score is based on two assumptions: 1) a research group conveys reputation to a publication
% venue proportionally to its own reputation. 2) a publication venue conveys reputation to a
% research group proportionally to its own reputation. Considering that these assumptions are
% valid, the framework of reputation flows assigns this reputation quantity to actors based on
% the publication patterns of the reputation sources. For example, if an institution is interested
% in ranking CS conferences according to the conference’s reputation among researchers in the
% field of Natural Language Processing, it can choose NLP researchers as reputation sources.
% This will yield a ranking that is different from the one produced using a more diverse set of
% reputation sources. We added Subsection 5.2 for an experimental analysis of how rankings
% can be affected by the choice of reputation sources.
% In this article, we used a selection of CS departments that are generally considered reputable
% 
% among computer scientists to produce a ranking of general reputation, but the usage of P-
% score is not limited to this selection. In the ICTIR article by Ribas et. al. [1], we use the
% 
% same selection of reputation sources to demonstrate the effectiveness of P-score compared to
% ground truths on venue ranking.
% The focus of the present work is to show that different indices can capture different aspects
% of the scientific relevance of a venue (which we call popularity and reputation) and, by
% comparing these indices, which are similar but not identical, we can highlight interesting
% cases where measures based on citation counts may be insufficient to provide a good measure
...
total number of citations the Impact Factors were obtained from the Microsoft
